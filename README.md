...
Recall earlier sections where we defined scopes for our server app registration. Defined scopes were in turn hard-coded in our [application code](server/routers/heroes.py) to enforce
that the token associated with incoming requests had the specified scopes. Now imagine if we were to spin up our client API and attempt to access the [routes protected by our server deployed to Azure Web Apps](client/routers/heroes.py).
Without any scopes in our token, we would just be prompted with an HTTP status response code of 403 (Forbidden).

Navigate to **Api permissions** section under your client app registration. Notice the presence of an already existing permission. The **User.Read** permission for MS Graph is baked into any new app registration. It is used to read information about the signed-in user.
Proceed to click on **Add a permission**
...
Search for and add the openid permission under Microsoft Graph.

![screenshot](images/hvalfangst_client_api_permissions_graph_openid.png)

Click on **Add a permission** yet another time. This time instead of Microsoft Graph under Microsoft APIs, click on **APIs my organization uses**. Search for and select **Hvalfangst Server**.

![screenshot](images/hvalfangst_client_api_permissions_hvalfangst_search.png)

You will be prompted with the choice of three possible permissions: **Heroes.Read**, **Heroes.Write** and **Heroes.Delete**, which coincides with the scopes we created earlier for our server. Check the **Heroes.Read** box and click on **Add permissions**.


![screenshot](images/hvalfangst_client_api_permissions_hvalfangst_server_heroes_read.png)

The configured permissions should now display three permissions: Heroes.Read, openid and User.Read. The first permission belong to our custom API **Hvalfangst Server**, whereas the latter two belongs to the built-in **Microsoft Graph** API. There will be an orange triangle indicating a warning
in the status column for our custom permission **Heroes.Read**. Click on the dotted lines to the right of the warning label.

![screenshot](images/hvalfangst_client_all_permissions_added.png)

You will be prompted with the choice to grant **admin consent**. This is used to avoid to manually verify that you are indeed requesting permissions on every call to the authorization server. Click on **Yes**.

![screenshot](images/hvalfangst_client_grant_admin_consent_prompt.png)

The list of configured permissions should now all have status equal to **Granted for Default Directory**. 
...
If you have correctly set up your app registrations and
your env file reflect this, a new browser window will be opened on startup - as mentioned previously.
